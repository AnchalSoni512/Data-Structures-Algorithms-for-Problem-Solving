...
string findpre(vector<ll> suffarr, string str)
{
    string var="", newvar;
    int n = str.size();
    // cout<<"size: "<<n<<endl;
    for(int i=0; i< n-1; i++)
    {
        // cout<<suffarr[i]<<" "<<suffarr[i+1]<<endl;
        // cout<<str.substr(suffarr[i])<<" "<<str.substr(suffarr[i+1])<<endl<<endl;
        newvar = commonPrefix(str.substr(suffarr[i]),str.substr(suffarr[i+1]));
        // cout<<newvar<<" ";
        // cout<<var<<endl; 
        // cout<<suffarr[i]+suffarr[i+1]<<" ";
        // cout<<n-newvar.size()<<endl;
        if( (newvar.size() > var.size()) && (suffarr[i]+suffarr[i+1] == n-newvar.size()))
        {
            // cout<<"inside if: "<<newvar<<endl;
            var= newvar;
...
int main(){
    ...
    str1 = str1 + str;
    // cout<<str1<<endl;
    vector<ll> suffarr=buildSuffix(str1);
    // for(int i=0; i<suffarr.size(); i++)
    // cout<<suffarr[i]<<" ";
    // cout<<endl<<endl;

    string pre = findpre(suffarr, str1);
    int z = pre.size();
    // cout<<"here: "<<z<<endl;
    if(pre.size())
    {  
...
/* test string 
1.mississipi-> ississi
2.abcdacb -> a
3.abbbc-> bbb */
//not working for string dcabca